...
def display_metrics(df):
    ...
    if not df.empty:
        metrics = {
            "Front Oil P": ("oil_pressure", "PSI", st.session_state.get('minFrontOilPressure'), st.session_state.get('nominalFrontOilPressure'), st.session_state.get('highFrontOilPressure'), st.session_state.get('maxFrontOilPressure')),
            "Back Oil P": ("oil_pressure_2", "PSI", st.session_state.get('minRearOilPressure'), st.session_state.get('nominalRearOilPressure'), st.session_state.get('highRearOilPressure'), st.session_state.get('maxRearOilPressure')),
            "OAT": ("oat", "°F", 0.0, 0.0, 0.0, 0.0),
            ...
            "Metered Fuel P": ("fuel_pressure", "PSI", st.session_state.get('minMeteredFuelPressure'), st.session_state.get('nominalMeteredFuelPressure'), st.session_state.get('highMeteredFuelPressure'), st.session_state.get('maxMeteredFuelPressure')),
            "Unmetered Fuel P": ("fuel_pressure", "PSI", st.session_state.get('minUnmeteredFuelPressure'), st.session_state.get('nominalUnmeteredFuelPressure'), st.session_state.get('highUnmeteredFuelPressure'), st.session_state.get('maxUnmeteredFuelPressure')),
            ...
            "CDT": ("cdt", "°F", st.session_state.get('minCDT'), 0.0, 0.0, 0.0), 
...
def snap_button_handler(df):
    ...
    metered_fuel_pressure = int(df['fuel_pressure'].iloc[0])
    unmetered_fuel_pressure = int(df['fuel_pressure'].iloc[0])
...
#    ws['I4'] = edited_cells
   ...
   green_fill = PatternFill(start_color="CCFFCC", end_color="CCFFCC", fill_type="solid")  # Nominal (within range)
   yellow_fill = PatternFill(start_color="FFFF99", end_color="FFFF99", fill_type="solid")  # High (between nominal and high)
   red_fill = PatternFill(start_color="FF9999", end_color="FF9999", fill_type="solid")    # Exceeds maximum (above high)
   blue_fill = PatternFill(start_color="ADD8E6", end_color="ADD8E6", fill_type="solid")   # Below minimum
                    ...
                    if min_row <= actual_row <= max_row and min_col <= c_idx <= max_col:
                        ...
                        if 10 <= min_row <= 19 and isinstance(value, (int, float)):
                            ...
                            if threshold is not None:
                                ...
                                if value < min_val:
                                    ws.cell(row=min_row, column=min_col).fill = blue_fill
