...
def display_metrics(df, engine_type):
    ...
    if not df.empty:
        if engine_type == dataOnly:
            ...
            "Back Oil P": ("rear_oil_p", "PSI"),
            # Add Upper Deck Pressure
            "Oil Temp": ("oil_temperature", "°F"),
            "Fuel Flow": ("fuel_flow", "GPH"),
            "Metered Fuel P": ("fuel_pressure", "PSI"),
            "Unmetered Fuel P": ("fuel_pressure", "PSI"),
            "Volts": ("volts", "V"),
            "CDT": ("cdt", "°F"), 
            "Mag Drop": ("mag_drop", "")
            }
                            ...
                            if 10 <= min_row <= 19 and isinstance(value, (int, float)):
                                ...
                                if threshold is not None:
                                    ...
                                    if value < min_val:
                                        ...
                                        ws.cell(row=min_row, column=min_col).fill = orange_fill
                                    else:
