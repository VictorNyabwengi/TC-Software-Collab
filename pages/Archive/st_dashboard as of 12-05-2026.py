...
save_xlxs_path = r"K:\New Test Cell\Test Sheets"
...
global engine_type, dataOnly, cylinderCount, turboStatus, thresholds
dataOnly = "Data Only"
...
#     prop_type = "2 Blade (Yellow Tipped)"
...
def test_sheet_df(path):
    ...
    """
    # Load xlxs file
    test_sheet_df = pd.read_excel(path, dtype=str)
...
def end_button_handler(df):
    ...
    #outside_temp = int(df['oat'].iloc[0])
    # st.session_state.test_sheet.loc[3, 'Unnamed: 8'] = outside_temp
    st.session_state.test_sheet.loc[4, 'Unnamed: 9'] = outside_temp
...
def display_test_sheet_data_editor(test_sheet_df):
    """
    Handles the UI and column configuration for the data editor
    """

    # Column Configuration
    column_configuration = {
        
        # Hide Unnamed Column Labels
        f"Unnamed: {i}": st.column_config.Column(label="",
                                                 width="small")
        for i in range(1, 21)                                   
    ...
    edited_test_sheet_df = st.data_editor(
        test_sheet_df, 
        column_config=column_configuration,
        use_container_width=True,
        hide_index=True,
        key="test_sheet_editor"
    )

    # Sync changes back to session state
    st.session_state.test_sheet = edited_test_sheet_df
...
def copy_to_excel(df, path):
    """
    Copies data from the session state dataframe into the formatted 
    excel template without losing the original formatting 
    """
    # 1. Load the existing template
    wb = openpyxl.load_workbook(path)
    # Assumes the test sheet is the first/active sheet
    ws = wb.active 

    # Iterate through the dataframe and write to the worksheet
    # Pandas index 0 is usually row 2
    # Will alight row_map here
    for r_idx, row in df.iterrows():
        for c_idx, value in enumerate(row):
            # Excel is 1-indexed
            # If df.index 0  is Row 1 in Excel, use r_idx + 1
            # If df.index 0 is Row 2 in Excel (after headers), use r_idx + 2
            excel_row = r_idx + 2
            excel_col = c_idx + 1

            # Standard write
            ws.cell(row=excel_row, column=excel_col).value = value
            # cell = ws.cell(row=excel_row, column=excel_col).value

            # try:
            #     # If value is NaN/None, openpyxl handles it fine
            #     cell.value = value
            # except AttributeError:
            #     # This triggers if cell is a merged cell (readonly)
            #     continue
            # except Exception as e:
            #     st.write(f"Error at {excel_row}, {excel_col}: {e}")

    # Merging Cells After Copying
    ...
    for merge_range in required_merges:
        try:
            ...
            # This prevents a crash if a range is already merged
            print(f"Skipping merge {merge_range}: {e}")
